const translations = {
  en: {
    app: {
      name: "TinyPush",
      currentState: "Current state",
      actionsTodayOne: "action today",
      actionsTodayOther: "actions today",
    },
    nav: {
      home: "Home",
      history: "History",
      settings: "Settings",
    },
    state: {
      STOPPED: {
        label: "Stopped",
        tone: "blocked, low energy",
        prompt: "Start with the smallest possible movement.",
      },
      NEUTRAL: {
        label: "Neutral",
        tone: "available, not yet engaged",
        prompt: "Choose one light action to create traction.",
      },
      ACTIVE: {
        label: "Active",
        tone: "engaged, ready to act",
        prompt: "Protect the momentum with a focused next step.",
      },
    },
    suggestions: {
      heading: "Suggested actions",
      cta: "Try a small action",
      close: "Close actions",
      searchPlaceholder: "Search or type your own action",
      addCustom: "Log this action",
      noMatches: "No matching suggestions",
      cooldownTitle: "Recent suggestions are on cooldown.",
      cooldownHint: "Wait a few minutes or type your own action above.",
      mode: "{state} mode",
      tagGoal: "Goal",
      tagWork: "Work",
      tagPeriod: "{period}",
      period: {
        morning: "Morning",
        afternoon: "Afternoon",
        evening: "Evening",
        night: "Night",
      },
      quickAction: "Quick action {number}",
      done: "Done",
      log: "Log",
      "stopped-breathe": "Take three slow breaths and put both feet on the floor.",
      "stopped-water": "Drink a glass of water or fill one now.",
      "stopped-open-space": "Clear one small surface within arm's reach.",
      "neutral-pick-one": "Write down one task that would make today easier.",
      "neutral-two-minute": "Set a two-minute timer and begin the easiest part.",
      "neutral-prepare": "Put the next tool or document directly in front of you.",
      "active-focus": "Work on one task for ten uninterrupted minutes.",
      "active-finish-line": "Define the next visible finish line before continuing.",
      "active-share": "Send a quick progress update or mark the task status.",
    },
    onboarding: {
      title: "Start tiny.",
      body: "Log one action and the app will update your state automatically.",
    },
    history: {
      title: "History",
      subtitle: "Your recent actions.",
      emptyTitle: "Nothing logged yet",
      emptyBody: "Tap a quick action on Home and it will show up here.",
      archiveAction: "Archive action",
      showMore: "Show {count} more",
      completedActions: "{count} completed actions",
      stateTransitions: "{count} state transitions",
    },
    settings: {
      title: "Settings",
      subtitle: "Tune TinyPush to your taste.",
      tabs: {
        context: "Context",
        preferences: "Preferences",
        data: "Data",
      },
      interface: "Interface",
      compact: "Compact",
      comfortable: "Comfortable",
      notifications: "Notifications",
      enableNotifications: "Enable notifications",
      tone: "Tone",
      soft: "Soft",
      direct: "Direct",
      appearance: "Appearance",
      system: "System",
      light: "Light",
      dark: "Dark",
      language: "Language",
      portuguese: "Portuguese",
      english: "English",
      objective: "Goal",
      objectivePlaceholder: "Example: finish my project report by Friday",
      objectiveHint: "This goal is used internally by AI to personalize action suggestions.",
      profession: "Profession",
      professionLabel: "Your profession",
      professionPlaceholder: "Example: Nurse, Software Engineer, Teacher",
      professionHint: "When you fill this field, TinyPush unlocks work-hours fields and profession-based suggestions.",
      suggestionOrder: "Suggestion order",
      suggestionOrderHint: "Top to down: the first item is the suggestion type that appears first.",
      suggestionOrderDragHint: "Drag and drop to reorder.",
      suggestionOrderDragItem: "Drag to reorder {item}",
      suggestionOrderPosition: "Priority {position}",
      suggestionType: {
        period: "Day period",
        profession: "Profession",
        goal: "Goal",
      },
      workHours: "Work hours",
      workStart: "Start",
      workEnd: "End",
      workHoursHint: "Work-focused suggestions are shown during this time window.",
      dataPortability: "Data portability",
      dataPortabilityHint: "Export your local data to a backup file or import a previous backup.",
      exportData: "Export local data",
      importData: "Import backup",
      reset: "Reset all data",
      version: "TinyPush · v2.0.0",
    },
    feedback: {
      dismiss: "Dismiss",
      completedSoft: "Nice work. Your state was recalculated from the latest report.",
      completedDirect: "Action logged. State recalculated.",
      completed: "Action completed. State recalculated from your latest report.",
      resetSoft: "All local data has been reset. You can start fresh.",
      resetDirect: "Local data reset completed.",
      reset: "All local data has been reset.",
      missingAction: "That action no longer exists for the current state.",
      updateAvailableSoft: "A new version is ready. Update when you want.",
      updateAvailableDirect: "New version ready.",
      updateAvailable: "A new version is ready.",
      updateNow: "Update",
      exportSoft: "Backup exported successfully.",
      exportDirect: "Backup exported.",
      exportError: "Unable to export backup.",
      importSoft: "Backup imported successfully.",
      importDirect: "Backup imported.",
      importError: "Invalid backup file.",
      historyItemArchivedSoft: "Action archived from view.",
      historyItemArchivedDirect: "History item archived.",
      historyItemArchiveError: "Unable to archive history item.",
    },
    loading: {
      label: "Behavior activation",
      title: "Restoring your local session",
    },
  },
  pt: {
    app: {
      name: "TinyPush",
      currentState: "Estado atual",
      actionsTodayOne: "ação hoje",
      actionsTodayOther: "ações hoje",
    },
    nav: {
      home: "Início",
      history: "Histórico",
      settings: "Ajustes",
    },
    state: {
      STOPPED: {
        label: "Parado",
        tone: "bloqueado, pouca energia",
        prompt: "Comece pelo menor movimento possível.",
      },
      NEUTRAL: {
        label: "Neutro",
        tone: "disponível, ainda sem engajamento",
        prompt: "Escolha uma ação leve para criar tração.",
      },
      ACTIVE: {
        label: "Ativo",
        tone: "engajado, pronto para agir",
        prompt: "Proteja o embalo com um proximo passo claro.",
      },
    },
    suggestions: {
      heading: "Ações sugeridas",
      cta: "Tentar uma pequena ação",
      close: "Fechar ações",
      searchPlaceholder: "Busque ou digite sua própria ação",
      addCustom: "Registrar esta ação",
      noMatches: "Nenhuma sugestão encontrada",
      cooldownTitle: "As sugestões recentes estão em pausa.",
      cooldownHint: "Aguarde alguns minutos ou digite sua própria ação acima.",
      mode: "modo {state}",
      tagGoal: "Objetivo",
      tagWork: "Trabalho",
      tagPeriod: "{period}",
      period: {
        morning: "Manhã",
        afternoon: "Tarde",
        evening: "Noite",
        night: "Madrugada",
      },
      quickAction: "Ação rápida {number}",
      done: "Feito",
      log: "Registrar",
      "stopped-breathe": "Respire fundo tres vezes e apoie os pes no chao.",
      "stopped-water": "Beba um copo de agua ou encha um agora.",
      "stopped-open-space": "Organize uma pequena area ao alcance da mao.",
      "neutral-pick-one": "Escreva uma tarefa que deixaria o dia mais facil.",
      "neutral-two-minute": "Coloque um timer de dois minutos e comece pela parte mais facil.",
      "neutral-prepare": "Coloque a próxima ferramenta ou documento na sua frente.",
      "active-focus": "Trabalhe em uma tarefa por dez minutos sem interrupção.",
      "active-finish-line": "Defina a próxima linha de chegada visível antes de continuar.",
      "active-share": "Envie uma atualização rápida ou marque o status da tarefa.",
    },
    onboarding: {
      title: "Comece pequeno.",
      body: "Registre uma ação e o app atualiza seu estado automaticamente.",
    },
    history: {
      title: "Histórico",
      subtitle: "Suas ações recentes.",
      emptyTitle: "Nada registrado ainda",
      emptyBody: "Toque em uma ação rápida no Início e ela aparecerá aqui.",
      archiveAction: "Arquivar ação",
      showMore: "Mostrar mais {count}",
      completedActions: "{count} ações concluídas",
      stateTransitions: "{count} transições de estado",
    },
    settings: {
      title: "Ajustes",
      subtitle: "Adapte o TinyPush ao seu gosto.",
      tabs: {
        context: "Contexto",
        preferences: "Preferências",
        data: "Dados",
      },
      interface: "Interface",
      compact: "Compacto",
      comfortable: "Confortável",
      notifications: "Notificações",
      enableNotifications: "Ativar notificações",
      tone: "Tom",
      soft: "Suave",
      direct: "Direto",
      appearance: "Aparência",
      system: "Sistema",
      light: "Claro",
      dark: "Escuro",
      language: "Idioma",
      portuguese: "Português",
      english: "Inglês",
      objective: "Objetivo",
      objectivePlaceholder: "Exemplo: finalizar meu relatório do projeto até sexta",
      objectiveHint: "Esse objetivo é usado internamente pela IA para personalizar sugestões de ações.",
      profession: "Profissão",
      professionLabel: "Sua profissão",
      professionPlaceholder: "Exemplo: Enfermeiro, Engenheiro de Software, Professor",
      professionHint: "Ao preencher este campo, o TinyPush libera os horários de trabalho e sugestões baseadas na profissão.",
      suggestionOrder: "Ordem das sugestões",
      suggestionOrderHint: "De cima para baixo: o primeiro item é o tipo de sugestão que aparece primeiro.",
      suggestionOrderDragHint: "Arraste e solte para reordenar.",
      suggestionOrderDragItem: "Arrastar para reordenar {item}",
      suggestionOrderPosition: "Prioridade {position}",
      suggestionType: {
        period: "Período do dia",
        profession: "Profissão",
        goal: "Objetivo",
      },
      workHours: "Horário de trabalho",
      workStart: "Início",
      workEnd: "Fim",
      workHoursHint: "Sugestões focadas em trabalho aparecem dentro dessa janela de horário.",
      dataPortability: "Portabilidade de dados",
      dataPortabilityHint: "Exporte seus dados locais para um arquivo de backup ou importe um backup anterior.",
      exportData: "Exportar dados locais",
      importData: "Importar backup",
      reset: "Redefinir dados",
      version: "TinyPush · v2.0.0",
    },
    feedback: {
      dismiss: "Fechar",
      completedSoft: "Bom trabalho. Seu estado foi recalculado pelo último registro.",
      completedDirect: "Ação registrada. Estado recalculado.",
      completed: "Ação concluída. Estado recalculado pelo último registro.",
      resetSoft: "Todos os dados locais foram redefinidos. Você pode recomeçar.",
      resetDirect: "Redefinição local concluída.",
      reset: "Todos os dados locais foram redefinidos.",
      missingAction: "Essa ação não existe mais para o estado atual.",
      updateAvailableSoft: "Uma nova versão está pronta. Atualize quando quiser.",
      updateAvailableDirect: "Nova versão pronta.",
      updateAvailable: "Uma nova versão está pronta.",
      updateNow: "Atualizar",
      exportSoft: "Backup exportado com sucesso.",
      exportDirect: "Backup exportado.",
      exportError: "Não foi possível exportar o backup.",
      importSoft: "Backup importado com sucesso.",
      importDirect: "Backup importado.",
      importError: "Arquivo de backup inválido.",
      historyItemArchivedSoft: "Ação arquivada da visualização.",
      historyItemArchivedDirect: "Item arquivado.",
      historyItemArchiveError: "Não foi possível arquivar o item do histórico.",
    },
    loading: {
      label: "Ativação comportamental",
      title: "Restaurando sua sessão local",
    },
  },
};

export const detectLocale = () => {
  const languages =
    typeof navigator === "undefined"
      ? []
      : [...(navigator.languages || []), navigator.language].filter(Boolean);

  return languages.some((language) => language.toLowerCase().startsWith("pt")) ? "pt" : "en";
};

export const createTranslator = (locale) => {
  const dictionary = translations[locale] || translations.en;

  return (key, values = {}) => {
    const value = key.split(".").reduce((current, part) => current?.[part], dictionary);
    const fallback = key.split(".").reduce((current, part) => current?.[part], translations.en);
    const template = typeof value === "string" ? value : fallback || key;

    return Object.entries(values).reduce(
      (text, [name, replacement]) => text.replaceAll(`{${name}}`, String(replacement)),
      template,
    );
  };
};
